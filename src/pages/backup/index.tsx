...
import { IBackupFormData, maintenanceBackupMediaOptions } from '../../types/pages/maintenance'
...
function BackupInfo() {
  ...
  const { trigger: downloadFileTrigger, isMutating: fileDownloadLoading } = useSWRMutation(
  ...
  return (
    <Page>
      ...
      {/* <FormActionButtonsContainer allowsShow>
        <Button size="large" onClick={handleSubmit} isLoading={isMutating || fileDownloadLoading}>
          <Icon icon={applyIcon} />
          <span>{t`Backup`}</span>
        </Button>
