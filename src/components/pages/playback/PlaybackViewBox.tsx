...
import { channelApi, homeApi } from '../../../api/urls'
...
const PlaybackViewBox: React.FC<IProps> = ({
  ...
  useEffect(() => {
    if (deviceId && selectedRecordedTime) {
      setIsStreamIsGenerated(false)
      setCountOfCheckIsStreamIsGenerated(0)
      getStreamUrl({
        Stream: currentStreamingType,
        Time: selectedRecordedTime,
        ChannelNo: deviceId,
      })
    }
  ...
  const headerActionButtons: IActionsButton[] = [
    {
      color: currentStreamingType === 'sub' ? 'danger' : 'primary',
      text: t`Main`,
      onClick: () => setCurrentStreamingType('main'),
      size: 'small',
    },
    {
      color: currentStreamingType === 'main' ? 'danger' : 'primary',
      text: t`Sub`,
  ...
  return (
    <div className="w-full pt-4 bg-white rounded-md md:px-4">
